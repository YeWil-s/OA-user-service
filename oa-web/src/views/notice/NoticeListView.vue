  ...
  <section class="page">
    ...
    <ModalDialog v-model="dialogOpen" :title="editing?.id ? '编辑公告' : '发布公告'">
      <div class="form-grid">
        ...
        <label class="form-item"><span class="form-label">目标 ID</span><input v-model="form.targetIdsText" class="field" placeholder="多个 ID 用英文逗号分隔" /></label>
...
function noticePayload(): Partial<Notice> {
  return {
    ...
    targetIds: form.targetType === 1 ? [] : parseIds(form.targetIdsText),
    status: form.status
  }
}
