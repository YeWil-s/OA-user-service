  ...
  <section class="page">
    ...
    <ModalDialog v-model="dialogOpen" :title="editing?.id ? '编辑人事变动' : '新增人事变动'">
      <div class="form-grid">
        ...
        <label class="form-item"><span class="form-label">目标部门</span>
          <select v-model.number="form.toDeptId" class="select">
            <option :value="undefined">不指定</option>
            <option v-for="d in allDepts()" :key="d.id" :value="d.id">{{ d.deptName }}</option>
          </select>
        </label>
        <label class="form-item"><span class="form-label">目标岗位</span>
          <select v-model.number="form.toPositionId" class="select">
            <option :value="undefined">不指定</option>
            <option v-for="p in positionList" :key="p.id" :value="p.id">{{ p.name }}</option>
          </select>
        </label>
...
import { assetApi, systemApi } from '@/api/services'
import type { DeptNode, StaffChange } from '@/api/types'

const rows = ref<StaffChange[]>([])
const deptTree = ref<DeptNode[]>([])
const positionList = ref<Array<{ id: number; name: string }>>([])
...
function flatDepts(tree: DeptNode[]): DeptNode[] {
  const r: DeptNode[] = []
  for (const d of tree) { r.push(d); if (d.children) r.push(...flatDepts(d.children)) }
  return r
}

function allDepts() { return flatDepts(deptTree.value) }
...
async function loadData() {
  try {
    const [deps, pos] = await Promise.all([
      systemApi.depts(),
      systemApi.positions({ pageNum: 1, pageSize: 200 })
    ])
    deptTree.value = deps
    positionList.value = (pos.records || pos.list || []).map((p: any) => ({ id: p.id, name: p.positionName || p.name }))
  } catch { /* ignore */ }
}
